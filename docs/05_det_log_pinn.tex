...
$u_\theta(\tau)$ and estimates the physical parameters by penalizing violations
...
$$
  \theta,\quad r,\quad K,
$$
where $\theta$ denotes all neural-network weights and biases. The unknowns in
the
stochastic PINN inverse problem are
$$
  \theta,\quad r,\quad K,\quad \sigma.
$$
The role of the neural trajectory $u_\theta$ is to provide a differentiable
...
$$
  u_\theta:[0,1]\rightarrow \mathbb R_+,
  \qquad
  \widehat y(t)=u_\theta(\tau(t)).
$$
...
$$
  u_\theta(\tau)=\operatorname{softplus}(W_o h_3+b_o)+10^{-6}.
$$
...
The final network layer is initialized so that $u_\theta(\tau)$ is approximately
...
$$
  ...
  =
  (t_{\max}-t_{\min})\,
  r u_\theta(\tau)\left(1-\frac{u_\theta(\tau)}{K}\right).
$$
...
$$
  ...
  \sum_{(i,j)\in\Omega}
  \left(
    \frac{u_\theta(\tau_i)-y_i^{[j]}}{s_y}
  \right)^2,
$$
$$
  \mathcal L_{\mathrm{init}}(\theta)
  =
  \frac{1}{J}
  \sum_{j=1}^J
  \left(
    \frac{u_\theta(0)-y_0^{[j]}}{s_y}
  \right)^2,
$$
and
$$
  \mathcal L_{\mathrm{phys}}(\theta,r,K)
  =
  \frac{1}{M}
  \sum_{m=1}^M
  \left[
    \frac{
      \dfrac{du_\theta}{d\tau}(\xi_m)
      -
      (t_{\max}-t_{\min})r u_\theta(\xi_m)
      \left(1-\dfrac{u_\theta(\xi_m)}{K}\right)
    }{s_y}
  \right]^2 .
$$
...
\begin{enumerate}
  ...
  \item Create the collocation grid
        ...
        enforced.
  \item Initialize the trajectory network $u_\theta$ and raw physical variables
  ...
  \item Evaluate $u_\theta(\xi_m)$ at the collocation points and use automatic
        differentiation to compute $du_\theta/d\tau$ at those same points.
  \item Form the residual
        $du_\theta/d\tau-(t_{\max}-t_{\min})r u_\theta(1-u_\theta/K)$ and
        compute
  ...
  \item After training, transform the final raw variables to
        $\widehat r$ and $\widehat K$, and evaluate $u_{\widehat\theta}(\tau)$
